...
def is_data_center(row: dict[str, str]) -> bool:
    """Only keep records whose sectors include data_center."""
    try:
        values = json.loads(row.get("sectors") or "[]")
    except json.JSONDecodeError:
        return False
    return isinstance(values, list) and "data_center" in values
...
def state_page(
    ...
    lines = [
        ...
        f"# {state} data center moratoria", "",
        f"Every local moratorium involving data centers in {state}, current through **{date_label}**.", "",
...
def state_index(by_state: dict[str, list[dict[str, str]]], as_of: dt.date) -> str:
    ...
    lines = [
        ...
        "# State-by-state data center moratorium index", "",
...
def main() -> None:
    ...
    all_inventory = load_csv(INV)
    inventory = [row for row in all_inventory if is_data_center(row)]
    legislation = load_csv(LEG)

    print(f"Original inventory: {len(all_inventory)} records")
    print(f"Data center inventory: {len(inventory)} records")
